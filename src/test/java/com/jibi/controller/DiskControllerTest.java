...
public class DiskControllerTest {
    ...
    public void testWrite() throws Exception {
        ...
        response = this.restTemplate.getForEntity("http://localhost:" + port + "/disk/write/MB/1", String.class);
        MatcherAssert.assertThat(response.getStatusCode(), is(HttpStatus.OK));

        response = this.restTemplate.getForEntity("http://localhost:" + port + "/disk/write/GB/0", String.class);
        MatcherAssert.assertThat(response.getStatusCode(), is(HttpStatus.OK));

        response = this.restTemplate.getForEntity("http://localhost:" + port + "/disk/write/TB/1", String.class);
